  ...
  useEffect(() => {
    const fetchProyectos = async () => {
      try {
        ...
        setProjects(data.projects || []);  // Asegurarse de que siempre se asigna un array
      } catch (error) {
                    ...
                    <Tab.Container id="projects-tabs" defaultActiveKey="first">
                      ...
                      <Tab.Content id="slideImUp" className={isVisible ? "animate_animated animate_slideInUp" : ""}>
                        <Tab.Pane eventKey="first">
                          <Row>
                            <button onClick={handleSelectProject} style={{ backgroundColor: 'blue', color: 'white', padding: '10px 20px', border: 'none' }}>
                              Agregar Proyecto
                            </button>
